...
public class TagController {
    ...
    public Result<String> del(@PathVariable Integer cid) {
        Tag data = tagService.getById(cid);
        if (data == null) return Result.error("该数据不存在");

        Boolean res = tagService.removeById(cid);
    ...
    public Result batchDel(@RequestBody List<Integer> cids) {
        Boolean res = tagService.removeByIds(cids);
    ...
    public Result<Tag> get(@PathVariable Integer cid) {
        Tag data = tagService.getById(cid);
